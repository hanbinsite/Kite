...
| ID | 任务 | 状态 | 优先级 |
|----|------|------|--------|
| M1 | tauri.conf.json 图标配置修复 | pending | critical |
| M2 | AiChatPanel XSS 修复 | pending | critical |
| M3 | Cmd+W dirty tab 确认对话框 | pending | critical |
| M4 | WS/MQTT/SSE 连接在 Tab 关闭时清理 | pending | critical |
| M5 | Proxy host 限制 localhost | pending | critical |

## High (18/18)

| ID | 任务 | 状态 | 优先级 |
|----|------|------|--------|
| M6 | SQLite schema_version 迁移逻辑 | pending | high |
| M7 | Cookie LIKE 改为精确 domain 匹配 | pending | high |
| M8 | reqwest::Client 连接池复用 | pending | high |
| M9 | 错误信息 per-tab 隔离 | pending | high |
| M10 | 硬编码中文字符串 i18n 化 | pending | high |
| M11 | 基础无障碍 (role/aria) | pending | high |
